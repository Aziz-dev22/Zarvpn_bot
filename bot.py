...
async def get_user_menu(user_id):
    ...
    if str(user_id) == str(config.ADMIN_ID):
        buttons.append([InlineKeyboardButton("⚙️ ورود به پنل وب مدیریت (با پسورد جدید)", url=f"{m_url}/")])
...
async def start(c, m):
    ...
    if not await is_subscribed(c, uid):
        ...
        markup = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("✅ بررسی عضویت", callback_data="check_sub_again")]
    ...
    await m.reply_text("🤖 به سیستم فروش هوشمند کانکشن خوش آمدید:\n📌 یک گزینه را انتخاب کنید:", reply_markup=await get_user_menu(uid))
    ...
    async with aiosqlite.connect("zarvpn_web.db") as db:
        if call.data == "back_to_main":
            ...
            if not plan:
                ...
                return
            if balance < plan[3]:
                await call.answer("❌ موجودی حساب شما کافی نیست!", show_alert=True)
                return
                
            await call.answer("⚙️ در حال اتصال به سرور و ساخت اکانت روی تمام اینباندها...", show_alert=True)
            v2_user = f"zar_{uid}_{pid}"
            
            # ارسال درخواست واقعی به X-UI سنایی
            ...
            if res and res.get("status") == "success":
                ...
                success_text = f"✅ **خرید با موفقیت انجام شد!**\n\n📦 پلن: {plan[0]}\n\n🔗 **ساب‌لینک اختصاصی شما (متصل به تمام پروتکل‌ها):**\n`{sub_link}`\n\n📌 لینک را کپی کرده و در برنامه v2rayNG وارد کنید."
                ...
                await call.edit_message_text(f"❌ خطا در صدور اکانت از سمت سرور X-UI:\n`{res.get('msg', 'Unknown Error')}`\n\nلطفاً موضوع را به پشتیبانی گزارش دهید.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="buy_menu")]]))
            ...
            try:
                ...
                srvs = []
            if not srvs: 
                await call.answer("❌ شما هیچ اشتراک فعالی ندارید!", show_alert=True)
                return
            ...
            if used == 1:
                await call.answer("❌ شما قبلاً تست رایگان دریافت کرده‌اید!", show_alert=True)
                return
            await call.answer("🎁 در حال ایجاد کانکشن تست...", show_alert=True)
            v2_user = f"test_{uid}"
            
            res = await panel_manager.create_account("xui", v2_user, 1, 1) # ۱ گیگ ۱ روز
            if res and res.get("status") == "success":
                sub_link = res["link"]
            ...
            text = f"👤 کاربر: @{usr[0]}\n🆔 آیدی: `{t_id}`\n💰 موجودی: **{usr[1]:,} تومان**"
            btns = [
                [InlineKeyboardButton("➕ افزایش ۵۰ هزار", callback_data=f"b_inc_{t_id}"), InlineKeyboardButton("➖ کاهش ۵۰ هزار", callback_data=f"b_dec_{t_id}")], 
            ...
            await db.commit()
            await call.answer("✅ موجودی کاربر شارژ شد.", show_alert=True)
